/** Простой калькулятор, создающий из строки массив, разбирающий его и вычисляющий результат выражения.
 * Поддерживает плавающую точку. Не поддерживает отрицательные числа и символы () для расставления приоритета операций.
 * @author Полина Рылова Junior Frontend Developer
 */
'use strict';
(function () {
  ...
  /** Проверяет, соответствует переданный в параметре элемент массива '*' или '/'.
  ...
  /** Проверяет наличие в массиве неподдерживаемых сочентаний символов '+*', '+/', '-*', '-/'.
   ...
   * @return {boolean} Возвращает true, если переданный item соответствует '*' или '/' и следует сразу за элементом, чей индекс был сохранён в previousIndex. Иначе - false.
   */
   ...
   * @throws {Error} Ошибка ввода. Первыми символами не могут быть "*" и "/".'
   * Если пользователь ввёл первым символом "*" или "/", то выражение является не поддерживаемым и выбрасывается исключение.
   * @throws {Error} Ошибка ввода. В конце выражения не должно быть знаков - только цифры.'
   * Если пользователь ввёл последним символом оператор, то выражение является не поддерживаемым и выбрасывается исключение.
   * @throws {Error} Ошибка ввода. Не поддерживаются сочетания "+*", "-*, "+/", "-/".'
   * Если пользователь ввёл подряд сочетание символов "+*", "-*, "+/", "-/", то выражение является не поддерживаемым и выбрасывается исключение.
   * @throws {Error} Ошибка ввода. Вы ввели неподдерживаемый символ или отправили пустую строку.
   ...
   * Если итерация последняя, то кладём сохранённый операнд в массив parsedArray.
   *
   * Если символ из перебираемого массива соответствует одному из элементов массива availableOperators ('+','-', '*', '/'),
   * то он сразу кладётся в массив без необходимости записи в переменную, так как всегда должен быть однозначным.
   * И на этой же итерации кладём сохранённый ранее операнд в массив parsedArray и очищаем переменную,
   * чтобы предыдущий операнд не сконкатенировался с новым.
   ...
   * Если он больше нуля, то возвращаем его в место вызова функции parseExpressions.
   */
   ...
   * В кейсе с плюсом (case '+') приводим строковые операнды к числу, чтобы избежать конкатенации, так ка нам нужна операция сложения.
   */
   ...
   * то проверяем есть ли уже сохранённое и не запушенное в массив lowPrecedenceExpressions зачение оператора.
   * Если есть, то сначала кладём в массив lowPrecedenceExpressions значение previousOperand,
   * затем operator (предыдущее сохранённое значение оператора), значение currentOperand перезаписываем в previousOperand
   * и только потом сохраняем значение текущего элемента в оператор.
   * Если же в переменной хранится значение пустой строки, то сразу записываем в переменную значение текущего элемента.
   *
   * Если текущий элемент цикла НЕ соответствует одному из элементов массива availableOperators,
   * то мы делаем следующие поверки данного операнда: если переменная previousOperand хранит пустую строку,
   * то сохраняем в неё значение текущего элемента, если же в ней уже хранится предыдущий операнд, то проверяем,
   * что хранится в последнем сохранённом операторе: если '+' или '-', то записываем текущее значение элемента
   ...
   * последний сохранённый previousOperand. Мы его пушим.
   * Если же нет, то останутся незапушенными последние previousOperand, operator и currentOperand,
   * поэтому после пуша previousOperand мы прооверяем наличие значения в переменной operator, отличного от пустой строки,
  ...
  /** Принимает массив строк и передает его параметром в функцию, которая вычисляет операции более высокого приоритета ('*' и '/')
   * и возвращает массив с операндами и операторами одного уровня приоритета.
   * @return {number} previousOperand - Число, полученное в результате вычислений.
   * Если возвращаемый массив содержит лишь один элемент (в случае ввода юзером выражения с только высокоприоритетными операциями),
   * то функция передает в точку своего вызова данный массив.
   ...
   * поэтому мы их передаем в функцию для вычислений результата операции по операнду и функция возвращает вычисленное значение в точку своего вызова.
   */
   ...
   * Полученное значение приводим к строке для единообразия типа параметра, передаваемого в функцию отображения результата пользователю.
   * Если ни на одном этапе не выбрасывается исключение,
   * то программа завершается отображением результата вычислений по введенному выражению в соответствующем блоке.
